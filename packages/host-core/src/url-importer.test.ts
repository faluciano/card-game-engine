...
function makeMockResponse(
  ...
  const { ok = true, status = 200, statusText = "OK", text = "{}", contentLength = null } = options;
  return {
    ok,
    status,
    statusText,
...
describe("importFromUrl", () => {
  ...
  describe("successful import", () => {
    it("fetches, parses, validates, and returns ruleset", async () => {
      ...
      mockFetch.mockResolvedValue(makeMockResponse({ text: JSON.stringify(rulesetData) }));
  ...
  describe("content-length guard", () => {
    it("rejects response when content-length exceeds limit", async () => {
      mockFetch.mockResolvedValue(makeMockResponse({ contentLength: "2000000" }));

      const result = await importFromUrl("https://example.com/large.json");

      expect(result).toEqual({
        ok: false,
        error: "Response too large: 2000000 bytes exceeds the 1048576 byte limit.",
      });
    });
  ...
  describe("body size guard", () => {
    it("rejects body larger than maxSizeBytes when no content-length header", async () => {
      ...
      mockFetch.mockResolvedValue(makeMockResponse({ text: largeBody }));

      const result = await importFromUrl("https://example.com/game.json", 100);
  ...
  describe("custom maxSizeBytes", () => {
    it("uses default maxSizeBytes of 1 MB", async () => {
      mockFetch.mockResolvedValue(makeMockResponse({ contentLength: "1048577" }));

      const result = await importFromUrl("https://example.com/game.json");

      expect(result).toEqual({
        ok: false,
        error: "Response too large: 1048577 bytes exceeds the 1048576 byte limit.",
      });
    });

    it("respects custom maxSizeBytes parameter", async () => {
      mockFetch.mockResolvedValue(makeMockResponse({ contentLength: "501" }));

      const result = await importFromUrl("https://example.com/game.json", 500);
  ...
  describe("invalid JSON", () => {
    it("returns error for malformed JSON response", async () => {
      mockFetch.mockResolvedValue(makeMockResponse({ text: "not-json{{{" }));
  ...
  describe("validation failure", () => {
    it("returns formatted Zod issues when validation fails", async () => {
      mockFetch.mockResolvedValue(makeMockResponse({ text: JSON.stringify({}) }));
